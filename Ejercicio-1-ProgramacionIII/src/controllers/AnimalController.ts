...
import { IAnimalFormData, IApiResponse, AnimalCategory, SystemMessage, IAnimal } from '../types/index.js';
...
export class AnimalController {
  ...
  private animales: IAnimal[] = [
    {
      id: '1',
      nombre: 'León',
      nombreCientifico: 'Panthera leo',
      categoria: AnimalCategory.MAMIFERO,
    ...
    },
    {
      ...
      categoria: AnimalCategory.REPTIL,
    ...
    },
    {
      ...
      categoria: AnimalCategory.INSECTO,
   ...
   */
  private buscarPorNombreModelo(nombre: string): IAnimal | undefined {
    return this.animales.find(animal =>
      animal.nombre.toLowerCase().includes(nombre.toLowerCase()) ||
   ...
   */
  private obtenerAleatorioModelo(): IAnimal {
   ...
   * Genera estadísticas
   */
  private obtenerEstadisticasModelo(): any {
    return {
      total: this.animales.length,
      porCategoria: Object.values(AnimalCategory).map(categoria => ({
        categoria,
   ...
   */
  private crearPersonalizadoModelo(nombre: string): IAnimal {
    const categorias = Object.values(AnimalCategory);
   ...
   */
  public mostrarPaginaPrincipal(req: Request, res: Response): void {
    ...
    res.render('index', {
      ...
      animalAleatorio,
      estadisticas,
      categorias: Object.values(AnimalCategory),
   ...
   */
  public buscarAnimal(req: Request, res: Response): void {
    try {
      const { nombreAnimal, emailUsuario, comentario } = req.body as IAnimalFormData;
   ...
   */
  public mostrarAnimal(req: Request, res: Response): void {
    try {
   ...
   */
  public apiObtenerAnimales(req: Request, res: Response): void {
    try {
   ...
   */
  public apiBuscarAnimal(req: Request, res: Response): void {
    try {
      ...
      const response: IApiResponse = {
        success: true,
        message: animal ? SystemMessage.SUCCESS : SystemMessage.NOT_FOUND,
        data: animal || null,
        timestamp: new Date(),
        code: animal ? 200 : 404
      };
  ...
  public mostrarAcercaDe(req: Request, res: Response): void {
